<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-slate-100 leading-tight">Messages</h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-800 dark:text-slate-100 p-6 rounded shadow-sm">
                <div class="flex items-center justify-between mb-4 flex-col sm:flex-row gap-2">
                    ...
                    <div class="flex gap-2 items-center flex-wrap">
                        <form method="GET" action="{{ route('messages.index') }}" class="flex gap-2 items-center flex-wrap">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        <thead class="bg-gray-50 dark:bg-slate-900">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase">Recipient</th>
                        ...
                        <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                            ...
                            <tr>
                                <td class="px-4 py-3">{{ $m->recipient_number ?? '—' }}</td>
                                <td class="px-4 py-3">{{ ucfirst($m->status) }}</td>
                                <td class="px-4 py-3">{{ $m->sent_at ? $m->sent_at->diffForHumans() : '—' }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('messages.show', $m) }}" class="px-2 py-1 bg-gray-200 rounded dark:bg-slate-700">View</a>
                                </td>
                            </tr>
                            @empty
                            <tr><td class="px-4 py-6 text-center" colspan="4">No messages yet.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-gray-700 dark:text-slate-400">
                        Showing <span class="font-medium">{{ $messages->firstItem() ?? 0 }}</span> 
                        to <span class="font-medium">{{ $messages->lastItem() ?? 0 }}</span> 
                        of <span class="font-medium">{{ $messages->total() }}</span> messages
                    </div>
