        ...
        <form action="{{ route('contacts.update', $contact) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input name="name" value="{{ old('name', $contact->name) }}" class="mt-1 block w-full rounded p-2 border border-slate-600 bg-slate-700 text-slate-100" />
                    @error('name') <p class="text-red-400 text-sm">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm text-slate-200">Phone</label>
                    <input name="phone" value="{{ old('phone', $contact->phone) }}" class="mt-1 block w-full rounded p-2 border border-slate-600 bg-slate-700 text-slate-100" />
            ...
            <div class="mt-4">
                ...
                <select name="contact_group_id" class="mt-1 block w-full rounded p-2 border border-slate-600 bg-slate-700 text-slate-100">
                    ...
                    @foreach($groups as $id => $name)
                        <option value="{{ $id }}" @if($contact->contact_group_id == $id) selected @endif>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
