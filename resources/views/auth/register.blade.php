<x-guest-layout>
    <div class="text-center mb-6">
        ...
        <p class="text-sm text-gray-300">Sign up to start creating campaigns and sending messages</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div>
            ...
            <x-text-input id="name" class="mt-1 block w-full rounded-md bg-white/5 border border-white/10 text-white placeholder-gray-400" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
        ...
        <div>
            ...
            <x-text-input id="phone" class="mt-1 block w-full rounded-md bg-white/5 border border-white/10 text-white placeholder-gray-400" type="text" name="phone" :value="old('phone')" required autocomplete="tel" />
            <x-input-error :messages="$errors->get('phone')" class="mt-2 text-sm text-red-400" />
        </div>

        <div>
            <label for="email" class="block text-sm font-medium text-gray-200">Email</label>
            <x-text-input id="email" class="mt-1 block w-full rounded-md bg-white/5 border border-white/10 text-white placeholder-gray-400" type="email" name="email" :value="old('email')" autocomplete="email" />
        ...
        <div>
            <x-primary-button class="w-full py-2 !bg-blue-500 hover:!bg-blue-600 focus:!bg-blue-600 active:!bg-blue-700 focus:!ring-blue-400">{{ __('Register') }}</x-primary-button>
        </div>

        <p class="text-center text-sm text-gray-300">Already registered? <a href="{{ route('login') }}" class="text-indigo-300 hover:underline">Sign in</a></p>
    </form>
</x-guest-layout>
